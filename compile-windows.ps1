# Windows compile script for KrankyBearTetris / tetris-windows (Windows build + optional packaging)
...
Write-Host "KrankyBear Tetris - Windows Compile Script" -ForegroundColor Cyan
...
Remove-Item -Path (Join-Path bin 'tetris') -Force -ErrorAction SilentlyContinue
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Current icon reference: ../Resources/Images/KrankyBearVikingHelmet.png" -ForegroundColor Yellow
    Write-Host ""
} else {
...
if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
    ...
    go build -ldflags="$ldflags" -trimpath -o bin/tetris-windows.exe
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($Package) {
            ...
            try {
                Copy-Item -Path (Join-Path $PSScriptRoot "bin/tetris-windows.exe") -Destination (Join-Path $PSScriptRoot "tetris-windows.exe") -Force
            } catch {
            ...
            if (Test-Path $InnoPath) {
                & "$InnoPath" "Inno/KrankyBearTetris.iss"
                if ($LASTEXITCODE -eq 0) {
...
if ($All -or $Linux) {
    if ($env:OS -eq 'Windows_NT') {
        ...
        $env:GOOS = "linux"
        $env:GOARCH = "amd64"
        go build -ldflags="-s -w" -trimpath -o bin/tetris-linux
        if ($LASTEXITCODE -eq 0) {
...
if ($Current) {
    ...
    go build -ldflags="-s -w" -trimpath -o bin/tetris-windows
    if ($LASTEXITCODE -eq 0) {
...
if ($buildFailed) {
    ...
    Get-ChildItem -Path bin -Filter "*tetris*" | Format-Table Name, Length -AutoSize
    ...
    if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
        ...
        Write-Host "  go-winres extract bin\tetris-windows.exe --dir extracted_resources" -ForegroundColor Cyan
    }
}
