...
# fpm-based packager for KrankyBear Tetris
# - macOS: Packages bin/tetris-macos-{arch} as .pkg installer to /Applications
# - Linux: Packages bin/tetris-linux-amd64 as .deb and .rpm installers
# - macOS: Installs as KrankyBearTetris.app bundle structure
...
Environment variables (optional):
  ...
  URL         Project URL (default: https://github.com/amarillier/KrankyBearTetris)
  LICENSE     License (default: GNU GPL v3)
  REPLACES    Comma-separated list of packages this replaces (default: tetris)
...
URL=${URL:-"https://github.com/amarillier/KrankyBearTetris"}
...
build_package() {
  ...
  BIN_NAME="KrankyBearTetris"
  
  if [[ "$TYPE" == "mac" ]]
  then
    SRC_BIN="bin/tetris-macos-${PKG_ARCH}"
    ...
    SRC_BIN="bin/tetris-linux-amd64"
  ...
  then
    ...
    APP_BUNDLE="KrankyBearTetris.app"
    ...
    # Create symlink: template -> KrankyBearTetris in Contents/MacOS
    ln -s "$BIN_NAME" "$APP_BUNDLE/Contents/MacOS/tetris"
  ...
  then
    ...
    cp -X "$SRC_BIN" "$STAGING_DIR/tetris"
    cp -XR "$SRC_IMAGES" "$STAGING_DIR/Images"
    cp -XR "$SRC_SOUNDS" "$STAGING_DIR/Sounds"
    
    # Copy ReleaseNotes.txt and rename to ReleaseNotes-tetris.txt
    cp "$SRC_RELEASE_NOTES" "$STAGING_DIR/ReleaseNotes-tetris.txt"
    # Set proper permissions (644 = rw-r--r--)
    chmod 644 "$STAGING_DIR/ReleaseNotes-tetris.txt"
    ...
    # Create symlink: template -> tetris (similar to macOS)
    # Use relative path - RPM will handle it correctly
    ln -sf "tetris" "$STAGING_DIR/template"
    ...
    SRC_BIN="$STAGING_DIR/tetris"
    SRC_SYMLINK="$STAGING_DIR/template"
    SRC_IMAGES="$STAGING_DIR/Images"
    SRC_SOUNDS="$STAGING_DIR/Sounds"
    SRC_RELEASE_NOTES_STAGED="$STAGING_DIR/ReleaseNotes-tetris.txt"
  ...
  COMMON_ARGS=(
    ...
    --url "$URL"
    --license "$LICENSE"
    --description "KrankyBear Tetris - A cross-platform GUI Tetris game"
    -f
  )
  ...
  then
    ...
    PKG_OUTFILE="$OUTDIR/tetris_${VERSION}-${ITERATION}_${PKG_ARCH}.pkg"
    echo "Building macOS .pkg ($PKG_ARCH) -> $PKG_OUTFILE..."
    
    # macOS .app bundle structure:
    # /Applications/KrankyBearTetris.app/Contents/Info-plist.txt (sample Info.plist)
    # /Applications/KrankyBearTetris.app/Contents/MacOS/KrankyBearTetris (executable)
    # /Applications/KrankyBearTetris.app/Contents/MacOS/Resources/Images (resources)
    # /Applications/KrankyBearTetris.app/Contents/MacOS/Resources/Sounds (sounds)
    # Note: App looks for Resources/Images and Resources/Sounds relative to executable
    APP_NAME="KrankyBearTetris.app"
    ...
    FPM_FILES=(
      ...
      "$APP_BUNDLE/Contents/MacOS/KrankyBearTetris=$MACOS_DIR/KrankyBearTetris"
    ...
    ./setIcon.sh Resources/Images/KrankyBearVikingHelmet.png "$PKG_OUTFILE"
    echo ""
    ...
    DEB_OUTFILE="$OUTDIR/tetris_${VERSION}-${ITERATION}_${DEB_ARCH}.deb"
    RPM_OUTFILE="$OUTDIR/tetris_${VERSION}-${ITERATION}_${RPM_ARCH}.rpm"
    ...
    fpm \
      "${DEB_ARGS[@]}" \
      --package "$DEB_OUTFILE" \
      "$SRC_BIN=/opt/local/bin/tetris" \
      "$SRC_SYMLINK=/opt/local/bin/template" \
      "$SRC_IMAGES=/opt/local/bin/Resources/Images" \
      "$SRC_SOUNDS=/opt/local/bin/Resources/Sounds" \
      "$SRC_RELEASE_NOTES_STAGED=/opt/local/bin/Resources/ReleaseNotes-tetris.txt" \
