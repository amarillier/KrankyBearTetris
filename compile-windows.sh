...
echo "KrankyBear Tetris - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/tetris-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/tetris-windows.exe
then
...
ls -lh bin/tetris-windows.exe || true
...
then
  ...
  cp -f bin/tetris-windows.exe ./KrankyBearTetris.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearTetris.iss || {
