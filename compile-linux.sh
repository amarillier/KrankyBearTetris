...
echo "KrankyBear Tetris - Linux Compile Script"
echo "========================================="
echo ""
...
then
    CC=aarch64-linux-gnu-gcc GOOS=linux GOARCH=arm64 CGO_ENABLED=1 go build -ldflags="-s -w" -trimpath -o bin/tetris-linux-arm64
    if [ $? -eq 0 ]
    then
...
echo ""
echo "Done. Binaries:"
ls -lh bin/tetris-linux-* 2>/dev/null || echo "  (no binaries found)"

# Copy amd64 to current directory for easy testing (if it exists)
if [ -f bin/tetris-linux-amd64 ]
then
    cp bin/tetris-linux-amd64 ./template
fi
...
then
    ...
    then
        echo "AMD64 binary:"
        ldd bin/tetris-linux-amd64 | grep -E "libasound|libGL|libX11" || true
    fi
...
send_binaries() {
    ...
    then
        ...
        local binaries=$(ls bin/tetris-linux-* 2>/dev/null)
    ...
    then
        ...
        echo "  scp $remote_user@$remote_host:$(pwd)/bin/tetris-linux-* ./bin/"
        echo ""
